...
client.on("guildMemberAdd", async member => {
  ...
  let Şüpheli = `${member} adlı kullanıcının hesabı güvenli değil!`;
   ...
   var ce1 = ['**Hoşgeldin Umarım Pizzaları Getirmişsindir.**', '**Sende Geldiğine Göre Parti Başlıyabilir.**', '**Geldiğine Çok Sevindik.**', '**Merhaba desene!**', '**Hepimiz Seni Bekliyorduk.**']
...
client.channels.get("759495512151162964").send(alp)
  });
...
client.channels.get("759497647173795880").send(hüso)
...
client.on("guildCreate", async guild => {
 ...
 yaprk.send('Sağol| **Beni Eklediğiniz İçin Teşekkürler!** |  \n\n `=>` **d!yardım** yazarak komutlarıma erişebilirsiniz!')
...
client.on("guildMemberAdd", async(member) => {
  ...
  if(otorolkanal && client.channels.has(otorolkanal)) {
    await client.channels.get(otorolkanal).send(`  \`${member.user.tag}\`  **Sunucuya Katıldı!  \`${member.guild.roles.get(otorol).name}\`  rolü başarıyla verildi.**  **Sunucu şu anda**  \`${member.guild.members.size}\`  **kişi!**`)
  };
});
...
client.on('guildMemberAdd', async member => {
  ...
  .setFooter(`${client.user.username} `, client.user.avatarURL)
   client.channels.get("775762329710362694").send(embed);
  })
